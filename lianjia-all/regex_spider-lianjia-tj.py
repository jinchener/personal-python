...
class regexSpider:
    ...
    def analysis(self , page):
        ...
        for i in list:
            ...
            rent = i.find ( "div" , "col-1" ).find ( "a" ).get_text ( ).encode ( 'utf-8' ).replace ( "正在出租" ,
                                                                                                     "" ).strip ( )
            ...
            year = lal.get_text ( ).encode ( 'utf-8' ).split ( "/" )[ 1 ].replace ( "建造" , "" ).strip ( )
            ...
            total = name + "|" + rent + "|" + locate1 + "|" + locate2 + "|" + price + "|" + sale + "|" + year + "|" + school + "|" + subline.strip ( ) + "|" + subst + "\n"
                ...
                if (soup != None):
                    ...
                    for link in links:
                        ...
                        if subUrl != None:
                            ...
                            # print "subUrl : "+subUrl
                            if (downUrlReg.match ( subUrl ) != None) and (ignoreUrlReg.match(subUrl) == None):  # 需要且不是过滤链接
    ...
    def spiderWorking(self , startUrlList , downUrlRegex , writeUrlRegex , ignoreUrlRegex , writeFileName , deepMax):
        ...
        file_object_Z.write("小区名|出租数|所属区|区域|均价|在售数|年份|学区|地铁线|地铁站")
        file_object_Z.write("\n")
...
if __name__ == "__main__":
    ...
    startUrlList = [ "http://tj.lianjia.com/xiaoqu/" ]
    ...
    writeUrlRegex = r"(http://tj.lianjia.com/xiaoqu/([a-z]{2,20})/pg[0-9]{0,4})"
    # "(http://pinyin\.sogou\.com/dict/cell\.php\?id=.*)|(http://download\.pinyin\.sogou\.com/dict/download_cell\.php.*)"
    ignoreUrlRegex = "(.*author\.php.*)|(.*search\.php.*)|(.*goto\.php.*)|(.*#top)|(.*rs.*)|(.*j[0-9]{1,10}.*)|(.*y[0-9]{1,10}.*)|(.*w[0-9]{1,10}.*)|(.*t[0-9]{1,10}.*)|(.*s[0-9]{1,10}.*)|(.*p[0-9]{1,10}.*)|(.*su[0-9]{1,10}.*)|(.*sc[0-9]{1,10}.*)|(.*cro[0-9]{1,10}.*)"
